...
public struct RGBAColor: Sendable, Equatable {
    ...
    public func mixed(towards other: RGBAColor, amount: Double) -> RGBAColor {
        ...
        func lerp(_ from: UInt8, _ to: UInt8) -> UInt8 {
            // Half **to even**, not Swift's default half-away-from-zero.
            //
            // Python's `round()` is half-to-even, and `application/palettes.mix`
            // — the twin of this function, and what every derived style in the
            // other application is built from — inherits that. A channel landing
            // exactly on 40.5 is 40 there and would be 41 here, so a derived sea
            // mark or contour could come out a unit off in one channel and no
            // fixture would catch it: both sides of every parity check are
            // generated by the implementation they check.
            //
            // `PaletteSheet.mix` already rounds this way for exactly this
            // reason. This is the same decision, applied to the other copy.
