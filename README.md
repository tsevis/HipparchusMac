...
[**Hipparchus 0.4.7**](https://github.com/tsevis/HipparchusMac/releases/latest)
...
```sh
Scripts/make-dmg.sh          # → dist/Hipparchus-0.4.7-<sha>.dmg
```
...
- **Palettes** — against `Scripts/build-style-packs.py`, whose `sheet()` is the
  same derivation and ships the five style packs. Two copies of one function
...
Five packs live in `Plugins/`, generated by `Scripts/build-style-packs.py`:
**Tsevis Palette** (two styles and five Ionian islands), **Nautical**,
**Duotone Press**, **High Contrast** and **Cartographers** (seven presets
named for people who drew the world, or went and looked at it — the same
colours as the runtime `--palette` catalogue, so a plugin export and a
recoloured preset cannot disagree). Every one derives all thirty-seven
